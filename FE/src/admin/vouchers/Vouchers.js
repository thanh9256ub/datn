...
import { Table, Input, Button, Badge, Spin, notification } from "antd";
...
const Vouchers = () => {
  ...
  const columns = [
    ...
    { title: "Mã", dataIndex: "voucherCode", key: "voucherCode" },
    { title: "Tên khuyến mại", dataIndex: "voucherName", key: "voucherName" },
    {
      title: "Loại giảm giá",
      dataIndex: "discountType",
      key: "discountType",
      render: (type) => (
        <Badge color={type === 0 ? "blue" : "green"} text={type === 0 ? "Theo số tiền" : "Theo %"} />
      ),
    },
    { title: "Giá trị tối thiểu", dataIndex: "minOrderValue", key: "minOrderValue" },
    { title: "Giá trị giảm", dataIndex: "discountValue", key: "discountValue" },
    { title: "Số lượng", dataIndex: "quantity", key: "quantity" },
    { title: "Giảm tối đa", dataIndex: "maxDiscountValue", key: "maxDiscountValue" },
    ...
    { title: "Ngày kết thúc", dataIndex: "endDate", key: "endDate" },
    {
      ...
      render: (status) => (
        <Badge
          color={status === 1 ? "green" : status === 0 ? "red" : "orange"}
          text={status === 1 ? "Đang hoạt động" : status === 0 ? "Đã hết hạn" : "Chưa kích hoạt"}
        />
      ),
    },
  ];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <Table columns={columns} dataSource={filteredVouchers} rowKey="id" pagination={{ pageSize: 10 }} />
      )}
    </div>
  );
};
